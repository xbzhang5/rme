...
The `cyclones` dataset in the `dobson` package (@tbl-cyclones-data) records "the number of tropical cyclones during a season from November to April in Northeastern Australia" (c.f. @dobson2018introduction §1.6.5 and `help(cyclones, package = "dobson")`). @fig-dobson-cyclone-time-series graphs the number of cyclones (y-axis) by season (x-axis). Let's use $Y_i$ to represent these counts, where $i$ is an indexing variable for the seasons and $Y_i$ is the number of cyclones in season $i$.

Suppose we want to learn about how many cyclones to expect per year.
