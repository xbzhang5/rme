...
**Logistic regression** is a framework for modeling a *binary outcome variable*, conditional on one or more *predictors* (a.k.a. *covariates*).
...
* vital status (dead vs alive) at a given time point 
* disease status (healthy vs diseased) at a given time point
* spam email vs not spam

In many health sciences applications, binary outcomes are defined for a *specific time point*, such as the end of a study, one year after exposure to a pathogen, or one month after treatment.

Many of these outcomes have a corresponding *time-to-event* variable. Time-to-event variables are modeled using survival analysis, covered in 

Logistic regression uses the Bernoulli family of probability distributions to model the outcome variable, conditional on  each set of covariate values. 
