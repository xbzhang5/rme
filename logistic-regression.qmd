...
### binary variable
A **binary variable** is a random variable which has only two possible values (such as diseased/healthy) in its range.
:::
...
But we can validly compute and use the exposure odds ratio, which is the odds for exposure in the disease group divided by the odds for exposure in the non-diseased group (because exposure can be treated as random). And these two odds ratios are mathematically equivalent, as we saw in @sec-OR-props.
