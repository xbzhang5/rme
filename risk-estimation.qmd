...
## Illustration: Oral Contraceptive Use and Heart Attack (MI) over 3 years
...
Review: estimate the probabilities of MI for OC users and non-OC users.

:::{.solution}

$$\hat{p}(MI|OC) = \frac{13}{5000} = `r 13/5000`$$

$$\hat{p}(MI|\neg OC) = \frac{7}{10000} = `r 7/10000`$$
...
:::{#def-RR}
### Relative risk ratio
When comparing the probability of an outcome between two sets of predictor values, the **relative risk**, also called the **risk ratio**, is the ratio of those probabilities.

That is, if $p(Y=1|X=x_1) = \pi_1$ and $p(Y=1|X=x_2) = \pi_2$, then $RR(2,1) = \frac{\pi_2}{\pi_1}$.
:::
...
In logistic regression, we make use of a transformation (rescaling) of probability, called *odds*.
...
:::{#exm-odds}
## Calculating odds
Above, we estimated that the probability of MI, given OC use, is $\pi(OC) \eqdef \Pr(MI|OC) = `r 13/5000`$. If this estimate is correct, then the odds of MI, given OC use, is:
...
For example, in @exm-odds, the probability and odds differ by `r abs(pi1 - odds1)`.
...
:::{#def-inv-odds}
The **inverse odds function**, $$\pi(\omega)\eqdef \frac{\omega}{1+\omega}$$
