...
For models in the "exponential family" of distributions, which includes the Gaussian, Poisson, Bernoulli, Binomial, Exponential, and Gamma distributions, $\einf{\theta} = \E{-\ell''(X|\theta)}$, so we estimated $\mathcal I(\theta)$ using either $\mathcal I(\theta)|_{\theta = \hthml}$ or $-\ell''(\vec x |\theta)|_{\theta = \hthml}$.
...
$$
\hat\beta \dot \sim t_{n-p}(\beta, \einf{\beta}^{-1})
$$ 
