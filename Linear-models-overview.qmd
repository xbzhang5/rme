...
- @dobson2018introduction, Chapters 2-6
...
Suppose we want to learn about the distributions of birthweights (*outcome* $Y$) for (human) babies born at different gestational ages (*covariate* $A$) and with different chromosomal sexes (*covariate* $S$) (@dobson2018introduction Example 2.2.2).
...
#| tbl-cap: "`birthweight` data (@dobson2018introduction Example 2.2.2)"
library(dobson)
...
#| fig-cap: "`birthweight` data (@dobson2018introduction Example 2.2.2)"
#| fig-height: 5
#| fig-width: 7
plot1 = bw |> 
  ggplot(aes(
...
-   $A$: chromosomal sex:
    -   $A = 0$ if female (XX)
    -   $A = 1$ if male (XY)
-   $S$: gestational age at birth (measured in weeks).

::: callout-note
Female is the **reference level** for the categorical variable $A$ (chromosomal sex). The choice of a reference level is arbitrary and does not limit what we can do with the resulting model; it only makes it more computationally convenient to make inferences about comparisons involving that reference group.
:::
...
\ba
E[Y|A = 0, S = 36] 
&= \beta_0 + \beta_S \cdot 0+ \beta_A \cdot 36 \\
&=  `r pred_female`
\ea
$$
:::
...
\ba
E[Y|A = 1, S = 36] 
&= \beta_0 + \beta_S \cdot 1+ \beta_A \cdot 36 \\
&=  `r pred_male`
\ea
$$
...
& E[Y|A = 1, S = 36] - 
E[Y|A = 0, S = 36]\\
&= (\beta_0 + \beta_S \cdot 1+ \beta_A \cdot 36) - (\beta_0 + \beta_S \cdot 0+ \beta_A \cdot 36) \\
&= \beta_S \\ 
...
$$
E[Y|A=a, S=s] = \beta_0 + \beta_A a+ \beta_S s + \beta_{AS} (a \cdot s)
$$ {#eq-model-interact}
...
$$
E[Y|A, S] = \beta_0 + \beta_A A+ (\beta_S + \beta_{AS} A) S
$$
...
$$E[Y|A = 0, X_2 = 36] = \beta_0 + \beta_S \cdot 0+ \beta_A \cdot 36 + \beta_{AS} \cdot (0 * 36) =  `r pred_female`$$ What's the mean birthweight for a male born at 36 weeks?
...
\ba
E[Y|A = 0, X_2 = 36]
&= \beta_0 + \beta_S \cdot 1+ \beta_A \cdot 36 + \beta_{AS} \cdot 1 \cdot 36\\
&= `r pred_male`
\ea
$$
...
$$
\begin{aligned}
& E[Y|S = 1, A = 36] - E[Y|S = 0, A = 36]\\ 
&= (\beta_0 + \beta_S \cdot 1+ \beta_A \cdot 36 + \beta_{AS} \cdot 1 \cdot 36)\\ 
&\ \ \ \ \  -(\beta_0 + \beta_S \cdot 0+ \beta_A \cdot 36 + \beta_{AS} \cdot 0 \cdot 36) \\
&= \beta_{S} + \beta_{AS}\cdot 36\\
&=  `r pred_male - pred_female`
\end{aligned}
$$
:::
...
$$
\E{Y|A=a, S=s} = 
\beta_M s + \beta_{AM} (a \cdot s) + 
\beta_F (1-s) + \beta_{AF} (a \cdot (1-s))
$$ {#eq-model-strat}
...
$$\hthml \eqdef \arg \max_\theta \ell(x|\theta)$$ {#eq-mle}

::: note
Recall: the second equality in @eq-lik relies on the assumption that the $X_i$s are mutually independent.
:::
...
- $\einf{\theta} \eqdef \E{\oinf{\vec X;\theta}}$
- $\oinf{\vec X;\theta} \eqdef -\ell''(\vec X;\theta)$

We can estimate $\einf{\theta}$ using either $\einf{\hthml}$ or $\oinf{\vec x; \hthml}$.

So we can estimate the standard error of $\hth_k$ as:

$$
\HSE{\hth_k} = \sqrt{\inv{\paren{\heinf{\hthml}}}_{kk}}
$$
...
### MLEs for Linear Regression {.smaller}
...
$$
\frac{\hat\beta_j}{\heinf{\beta}}_j \dist t_{n-p}
$$ 
---
...
$P(|\hat\beta_{AS}| > |`r coef(bw_lm2)["sexmale:age"]`| \mid H_0)$ = ?
...
\\
&= P\left(
\left| \frac{\hat\beta_{AS}}{\hat{SE}(\hat\beta_{AS})} \right| > 
...
::: callout-note
Fig 2.4 panel (c) in @dobson2018introduction is a little different; they didn't specify how they produced it, but other statistical analysis systems do things differently from R.
:::
