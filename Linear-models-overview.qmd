...
Suppose we want to learn about the distributions of birthweights (*outcome* $Y$) for (human) babies born at different gestational ages (*covariate* $A$) and with different chromosomal sexes (*covariate* $S$) (@dobson18 Example 2.2.2).
...
#| tbl-cap: "`birthweight` data (@dobson18 Example 2.2.2)"
library(dobson)
...
#| fig-cap: "`birthweight` data (@dobson18 Example 2.2.2)"
#| fig-height: 5
#| fig-width: 7
plot1 = bw |> 
  ggplot(aes(
...
$$
\E{Y|A=a, S=s} = \beta_0 + \beta_A a+ \beta_M m + \beta_{AM} (a \cdot s)
$$
...
::: callout-note
Fig 2.4 panel (c) in @dobson18 is a little different; they didn't specify how they produced it, but other statistical analysis systems do things differently from R.
:::
...
:::{.notes}
For example, @tbl-iris-data shows the (in)famous `iris` data (@anderson1935irises), 
...
There are more options; see @dobson18 §6.4.1 and the [`codingMatrices` package](https://CRAN.R-project.org/package=codingMatrices)  [vignette](https://cran.r-project.org/web/packages/codingMatrices/vignettes/codingMatrices.pdf) (@venablescodingMatrices).
