...
:::{#def-resid-noise}
#### Residual noise in linear models

The **residual noise** in a regression model $p(Y|X)$ is the difference between an observed value $y$ and its distributional mean:

$$\epsilon(y) \eqdef y - \Exp{Y|X}$$
:::

Note that this is the same notation that we used for [errors](#estimation.qmd#def-error). Each observation $y$ can be viewed as an estimate of $\Exp{Y|X}$ (albeit an imprecise one, individually, since $n=1$).

:::{#def-resid-fitted}
#### Residual of a fitted value
...
For Gaussian models, $e(\hat y)$ can be seen as the maximum likelihood estimate of the residual noise:
...
-   For Gaussian models: $$\widehat{SD}(e_i) \approx \hs$$
