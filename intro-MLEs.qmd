...
Let $p_\Theta(\vec X)$ be a probability model for the distribution of $\vec X$ with parameter vector $\Theta$.

Then the **likelihood** of parameter value $\theta$ (for model $p_\Theta(\vec X)$) is the *joint probability* of $\vec x$ given $\Theta = \theta$:
 
$$
\ba
\Lik(\theta) &\eqdef p_{\theta}(\vec X)
...
::: notes
The likelihood is a function that takes $\theta$ (and implicitly, $\vec X$) as inputs and outputs a single number, the joint probability of $\vec X$ for model $p_\Theta(\vec X)$ with $\Theta = \theta$.
:::
...
:::{.proof}
By definition of statistical independence.
:::
...
The **maximum likelihood estimate** of a parameter vector $\theta$ is the value of $\theta$ that maximizes the likelihood:

$$
\hthml = \arg \max_\theta \Lik(\theta)
$$
...
- $\einff{\theta} \eqdef \E{\oinff{\vec X;\theta}}$
- $\oinff{\vec X;\theta} \eqdef -\ell''(\vec X;\theta)$

We can estimate $\einff{\theta}$ using either $\einff{\hthml}$ or $\oinff{\vec x; \hthml}$.

So we can estimate the standard error of $\hth_k$ as:

$$
\HSE{\hth_k} = \sqrt{\inv{\paren{\heinff{\hthml}}}_{kk}}
$$
...
the vector of true parameters, and $\Theta = (M, \Sigma^2)$ is the vector of parameters as a random
vector.
...
$${Var}(X) \eqdef E\left\lbrack \left( X - E\lbrack X\rbrack \right)^{2} \right\rbrack = E\left\lbrack X^{2} \right\rbrack - \left( E\lbrack X\rbrack \right)^{2}$$

For any two-dimensional random variables, $X,Y$:

$${Cov}(X,Y) = E\left\lbrack (X - EX)(Y - EY) \right\rbrack = E\lbrack XY\rbrack - E\lbrack X\rbrack E\lbrack Y\rbrack$$

Therefore,
${Var}{(X)} = {Cov}(X,X) = E\lbrack XX\rbrack - E\lbrack X\rbrack E\lbrack X\rbrack = E\left\lbrack X^{2} \right\rbrack - \left( E\lbrack X\rbrack \right)^{2}$
...
In the Gaussian case, $i,j \in \left\{ \mu,\sigma^{2} \right\}$. 
...
$\oinff{\hthml;\vx}$ for convenience; there are
